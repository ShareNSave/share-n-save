...
namespace ZeroWaste.SharePortal.Controllers
{
    ...
    {
        readonly IContentItemRepository _finder;
        ...
        {
            var model = string.IsNullOrEmpty(tag) ? GetNews(0, 6) : GetNews(tag, 0, 6);
            return View(model);
        }

        public ActionResult Archive(int year)
        {
            var query = (Parameter.Below(CurrentPage) & Parameter.GreaterOrEqual("Published", new DateTime(year, 1, 1)) & Parameter.LessThan("Published", new DateTime(year + 1, 1, 1))).OrderBy("Published DESC");
            var news = _finder.Find(query).OfType<News>().ToList();
            var model = CreateModel(0, 1000, news);
